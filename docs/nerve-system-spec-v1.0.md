...
| 1.7 | 2026-07-21 | M7 build: evaluator v0 + approval queue realized (Section 7). Corrects a long-stale drift: Section 7 had said bookkeeping diffs auto-apply "up to a versioned per-run cap (default 5)" since v0, but that was superseded at the v1.0 review to cap 0/classify-only — the changelog row never landed, so spec and code disagreed until now. Cap 0 is the corrected default; raising it requires both a changelog row and an explicit enabling condition (≥N evaluator cycles at ≥95% unmodified owner approval of bookkeeping diffs). Section 7 also gains the findings-storage description: mutable status (a decision-in-progress queue), backed by an append-only, reason-carrying transition log as the actual source of truth — a deliberate departure from Section 6's run-record append-only pattern, justified because a finding is a pending decision, not history. First scheduled review executed manually against 29 real runs (one short of this milestone's own ≥30 DoD bar, stated explicitly in the evaluator's own output rather than silently absorbed) — produced 4 findings, including the residue-instrumentation gap (Section 7 requires residue analysis; the router never instruments it — approved as a structural finding, remedy routed as a task) and a dead-branch finding on `deferred` (rejected: a single never-fired branch is an absence observation, not a computed rate, and treating it as equivalently strong evidence overstates the claim). **Note:** the founding-principle paragraph for Section 1 was intended for this version bump but its source (`docs/nerve-founding-principle.md`) could not be located on this host as of this row landing — Section 1 is unchanged pending that content | Tiaan + Claude |
...
- **Hard boundary:** substantive diffs land in the owner approval queue and are never applied by the evaluator itself. Bookkeeping diffs *may* auto-apply, up to a versioned per-run cap — **default corrected to 0 (classify-only) at v1.7.** (This spec had said "default 5" since v0; that was superseded at the v1.0 review to cap 0/classify-only, but the changelog row never landed — every diff, bookkeeping or substantive, requires owner approval until the cap is explicitly raised.) Raising the cap above 0, or enabling auto-apply at all, requires **both** a changelog row **and** the enabling condition: at least N evaluator cycles (a starting value is set at build time, e.g. 4 — roughly a month at weekly cadence) at ≥95% owner approval of bookkeeping-class diffs, unmodified. This is a human decision gate, not a runtime threshold the evaluator adjusts itself. Approved (or auto-applied, once enabled) diffs are applied as new versions with the changelog updated. Rejected diffs are logged with the rejection reason (that reason is itself evaluator input next cycle).
...
**Findings storage (v1.7):** each evaluator cycle writes one row per finding or observation — target artifact, current/proposed value, the run ids that are its evidence, sample size (`n`) against that check's minimum-sample gate, an explicit **evidence-strength** field (structural / below-gate / adequate), category (bookkeeping/substantive), and a status that moves pending → approved/rejected/auto-applied. Unlike the run record (Section 6), this is **not** append-only — a finding is a decision-in-progress, not immutable history. What *is* append-only is the audit trail: every status transition is its own permanent, reason-carrying record, so a finding's current status is a convenience view over a history that is never overwritten. A check whose sample is below its gate is recorded as an observation (informational) rather than a finding (actionable) — it does not enter the approval queue.
